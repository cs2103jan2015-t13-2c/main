 ...
 */
 void Taskky::main() {
	...
	while (true) {
		...
		string feedback = executeCommand(userCommand, tempSave, fileName);
		showToUser(feedback);	
	}
}
...
*/
	 ...
	 string taskDetails = "";
	 chrono::system_clock::time_point taskStartTime = makeTimePoint(1971, 1, 1, 0, 0);
	 chrono::system_clock::time_point taskEndTime = makeTimePoint(1971, 1, 1, 0, 0);
	 chrono::system_clock::time_point taskDeadline = makeTimePoint(1971, 1, 1, 0, 0);
	...
	Task newTask = Task::Task(taskNumber, taskDetails, taskStartTime, taskEndTime, taskDeadline,
...
void Taskky::writetoFile(vector<Task> tempSave, string fileName) {
	 ...
	 for (iter = tempSave.begin(); iter != tempSave.end(); ++iter) {
		 file << "=============================" << endl;
		 file << "Task #" << setw(8) << setfill('0') << (*iter).getTaskNumber() << endl;
		 file << "=============================" << endl;
		 file << (*iter).getTaskType() << endl;
		 file << (*iter).getTaskDetails() << endl;
		 file << Taskky::asString((*iter).getTaskStartTime()) << endl;
		 file << Taskky::asString((*iter).getTaskEndTime()) << endl;
		 file << Taskky::asString((*iter).getTaskDeadline()) << endl;
...
chrono::system_clock::time_point Taskky::parseTime(string timeStr) {
	...
	cout << day << " " << mon;
	return (Taskky::makeTimePoint(2015, mon, day, hour, min));
}
...
string Taskky::asString(const std::chrono::system_clock::time_point& tp) {
...
chrono::system_clock::time_point Taskky::makeTimePoint(int year, int mon, int day, int hour, int min) {
	int sec = 0;
